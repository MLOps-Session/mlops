...
def result(request):
    ...
    b = aids(age=int(request.GET['age']), sex = int(request.GET['sex']),bmi = int(request.GET['bmi']),children = int(request.GET['children']),smoker = int(request.GET['smoker']),region = int(request.GET['region']))
    b.save()
